# COMP-SCR004-SAFETY-GUIDE — 신청 방법 3단계 + 안전 안내 배너

| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR004-SAFETY-GUIDE` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 31번 행(`COMP-SCR004-SAFETY-GUIDE`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **신청 방법 3단계 + 안전 안내 배너**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-004 | `/mates` | `src/components/scr004/SafetyGuide.tsx` |
...
신규 생성: `src/components/scr004/SafetyGuide.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 참가 요청 3단계 안내 + 안전 약속 CTA Banner("여행 조건 먼저 정리하기"→`/travel-tools`)

## Visual AC

- 3단계 안내 + CTA Banner 패턴
