...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 38번 행(`SHARED-DESIGN-TOKENS`, Category=SHARED)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **디자인 토큰(Tailwind 설정)**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| 전역 | (all routes) | `tailwind.config` 또는 `src/app/globals.css` |
...
수정: `src/app/globals.css`; 신규 생성: `src/styles/tokens.ts`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- D-001/DESIGN.md의 Color/Typography/Spacing/Radius/Shadow 토큰이 Tailwind 테마로 반영

## Visual AC

- 토큰 값이 D-001 §2~6과 정확히 일치
- 임의 색상 추가 없음(D-001 §21)
