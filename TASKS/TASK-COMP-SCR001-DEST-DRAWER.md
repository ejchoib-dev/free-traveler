...
| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR001-DEST-DRAWER` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 5번 행(`COMP-SCR001-DEST-DRAWER`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **여행지 상세 Drawer**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-001 | `/` | `src/components/scr001/DestinationDrawer.tsx` |
...
DATA-DESTINATIONS, COMP-SCR001-SAFETY-DRAWER

## Expected Files

신규 생성: `src/components/scr001/DestinationDrawer.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 소개·명소 5개 이상·추천 시기·1일/3일 일정·예산·교통·음식 3개 이상·에티켓·출처·수정일 모두 표시
- 해외는 하단 "국가 안전정보 보기" 버튼으로 안전정보 Drawer를 같은 스택 위에 이어 연다

## Visual AC

- Desktop 우측 슬라이드 520px / Mobile 하단 시트 92%, `shadow.card` 1단계만 사용(D-001 §12)
