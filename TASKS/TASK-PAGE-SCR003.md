...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 19번 행(`PAGE-SCR003`, Category=PAGE_OWNER)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **여행 준비 페이지 조립**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
REQ-FUNC-054, REQ-FUNC-064, REQ-FUNC-065, REQ-FUNC-070, REQ-FUNC-079, REQ-FUNC-080
...
COMP-SCR003-INTRO-TABS, COMP-SCR003-FLIGHT, COMP-SCR003-HOTEL, COMP-SCR003-MATE-WRITE, SHARED-LAYOUT
...
- Section 순서 Intro→탭→여행정보 Form→입력 요약·외부 이동→찾기 Tip 3→동행 작성 또는 로그인 안내·안전 안내가 이 순서로 조립되고
- 항공편·숙소·동행 구하기 3개 탭이 실제로 조립되어 각 탭이 독립된 입력·검증·완료 상태를 유지한다

## Visual AC

- Section 순서·데이터 출처(항공/숙소=클라이언트 상태만
- 동행 작성=DB-ACCESS) 고정. 최소 콘텐츠 수: 찾기 Tip 3개
- 탭 3개 전부 항상 노출. Lorem ipsum·'준비 중'·빈 Card 금지
- 미인증 상태는 안내 카드+CTA를 갖춘 완성형 Empty State

## Security/Privacy AC

- 항공·숙소 입력값(국가·지역·날짜)은 서버 DB·서버 로그·분석 이벤트·외부 URL 쿼리 어디에도 전달·저장하지 않는다(REQ-FUNC-017·025
- NF-017)
...
- TC-FUNC-080: `REQ-FUNC-080` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- 추가 검증: E2E-TRAVEL-TOOLS에서 이 Task의 결과를 다시 확인
...
- [ ] 'Verify'에 지정된 E2E-TRAVEL-TOOLS가 통과한다
