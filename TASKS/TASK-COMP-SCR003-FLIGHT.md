...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 21번 행(`COMP-SCR003-FLIGHT`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **항공 조건 입력·요약·외부 이동**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).

## Project Scope

Implementation Status: **IMPLEMENT** (`docs/PROJECT_SCOPE.md` 기준, `docs/UIUX_TRACEABILITY.md`와 일치)

PROJECT_SCOPE.md §5: 항공·숙소 조건 입력값은 서버 DB·서버 로그·분석 이벤트·URL 쿼리 어디에도 전달·저장하지 않는다. 전용 API Route/Server Action을 만들지 않고 클라이언트 상태만 사용한다.
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-003 | `/travel-tools` | `src/components/scr003/FlightTab.tsx` |
...
신규 생성: `src/components/scr003/FlightTab.tsx`
...
- Form 좌우분할(폼+안내)
- 오류는 색상+텍스트 병기(D-001 §10)

## Security/Privacy AC

- **입력값을 서버 DB·서버 로그·분석 이벤트·URL 쿼리에 전달·저장하지 않는다. 전용 API Route를 만들지 않는다.**(REQ-FUNC-017, NF-017)

## Test Cases

- TC-FUNC-011: `REQ-FUNC-011` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-012: `REQ-FUNC-012` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-013: `REQ-FUNC-013` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-014: `REQ-FUNC-014` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-015: `REQ-FUNC-015` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-016: `REQ-FUNC-016` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-017: `REQ-FUNC-017` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-018: `REQ-FUNC-018` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-054: `REQ-FUNC-054` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-NF-017: `REQ-NF-017` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- 추가 검증: UNIT-TRAVEL-DATES, E2E-TRAVEL-TOOLS에서 이 Task의 결과를 다시 확인
...
- [ ] 'Verify'에 지정된 UNIT-TRAVEL-DATES, E2E-TRAVEL-TOOLS가 통과한다
...
- **항공·숙소 입력값(국가·지역·날짜)을 서버 DB, 서버 로그, 분석 이벤트, 외부 URL 쿼리 어디에도 전달·저장하지 않는다.** 이 Task를 위한 전용 API Route/Server Action을 만들지 않는다.
