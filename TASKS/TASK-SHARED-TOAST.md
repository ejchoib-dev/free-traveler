# SHARED-TOAST — Toast 알림 시스템

| 항목 | 값 |
|---|---|
| Task ID | `SHARED-TOAST` |
| Category | SHARED |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 39번 행(`SHARED-TOAST`, Category=SHARED)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **Toast 알림 시스템**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| 전역 | (all routes) | `src/components/shared/Toast.tsx` |
...
신규 생성: `src/components/shared/Toast.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 참가 요청 접수/승인/거절/신고 처리 결과가 1분 이내 인앱 Toast로 표시
- 이메일 장애가 상태 변경을 롤백하지 않음

## Visual AC

- 색상+텍스트 라벨 병기(semantic color)
