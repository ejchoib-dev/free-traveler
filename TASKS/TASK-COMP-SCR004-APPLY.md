# COMP-SCR004-APPLY — 참가 요청 폼

| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR004-APPLY` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 28번 행(`COMP-SCR004-APPLY`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **참가 요청 폼**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
REQ-FUNC-034, REQ-FUNC-035, REQ-NF-019

## Screen / Route / Page Entry

| Screen | Route | Page Entry |
|---|---|---|
| SCR-004 | `/mates` | `src/components/scr004/ApplyForm.tsx` |
...
DB-ACCESS, COMP-SCR004-DETAIL, SHARED-AUTH-SETUP

## Expected Files

신규 생성: `src/components/scr004/ApplyForm.tsx`
...
- 최대 500자 비공개 메시지
- PENDING 저장
- 동일 사용자 중복 PENDING/ACCEPTED 차단
- 비로그인·성인미인증 사용자가 참가 요청을 시도하면 제출을 막고 로그인·성인인증 안내(→SCR-005)로 전환(Unauthorized, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
- 제출 실패(네트워크 오류·서버 오류) 시 값을 유지한 채 오류 메시지+재시도 버튼 표시(Error, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
...
- 추가 검증: UNIT-MATE-STATE, TEST-RLS-BASIC에서 이 Task의 결과를 다시 확인
...
- [ ] 'Verify'에 지정된 UNIT-MATE-STATE, TEST-RLS-BASIC가 통과한다
