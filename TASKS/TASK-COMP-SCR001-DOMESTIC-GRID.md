# COMP-SCR001-DOMESTIC-GRID — 국내 여행지 카드 그리드
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 3번 행(`COMP-SCR001-DOMESTIC-GRID`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **국내 여행지 카드 그리드**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-001 | `/` | `src/components/scr001/DomesticGrid.tsx` |
...
신규 생성: `src/components/scr001/DomesticGrid.tsx`
...
- 국내 여행지 6개 카드
- 빈 결과 시 조건 완화 안내+초기화 버튼

## Visual AC

- Next.js Image로 반응형·lazy load
- 카드 12px radius(D-001 §5·§9)
