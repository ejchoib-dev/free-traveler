...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 59번 행(`MANUAL-A11Y-CHECK`, Category=MANUAL_CHECK)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **접근성 수동 확인**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
- 키보드만으로 핵심 UC(UC-01~09) 100% 완료
- 스크린리더로 폼·모달·탭·알림 의미 확인

## Visual AC

- 포커스 링이 `:focus-visible`에서만 표시(D-001 §2)
...
- TC-NF-023: `REQ-NF-023` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-NF-025: `REQ-NF-025` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
