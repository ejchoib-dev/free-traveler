...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 26번 행(`COMP-SCR004-LIST`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **동행 모집글 목록**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-004 | `/mates` | `src/components/scr004/MateList.tsx` |
...
DB-ACCESS, COMP-SCR004-FILTER

## Expected Files

신규 생성: `src/components/scr004/MateList.tsx`
...
- 조회 중 목록 카드 스켈레톤 표시(Loading, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
- 조회 실패 시 재시도 버튼 표시(Error, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
