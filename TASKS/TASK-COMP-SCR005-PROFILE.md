...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 34번 행(`COMP-SCR005-PROFILE`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **프로필·성인확인**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-005 | `/account` | `src/components/scr005/ProfileTab.tsx` |
...
신규 생성: `src/components/scr005/ProfileTab.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 닉네임(필수)·연령대(필수)·성별(선택)·여행스타일(필수)·자기소개
- 성인 확인 상태 배지+미완료 시 CTA
- 조회 중 프로필 카드 스켈레톤 표시(Loading, `design-reference/UI_CONTRACT.md` SCR-005 상태 행)

## Visual AC

- 프로필 카드+수정 폼(D-001 §10)
