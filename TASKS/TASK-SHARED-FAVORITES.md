# SHARED-FAVORITES — 즐겨찾기(localStorage)

| 항목 | 값 |
|---|---|
| Task ID | `SHARED-FAVORITES` |
| Category | SHARED |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 41번 행(`SHARED-FAVORITES`, Category=SHARED)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **즐겨찾기(localStorage)**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| 전역 | (all routes) | `src/hooks/useFavorites.ts` |
...
신규 생성: `src/hooks/useFavorites.ts`
...
- 서버 저장 없이 브라우저별 localStorage만 사용
