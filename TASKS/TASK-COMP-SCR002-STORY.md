# COMP-SCR002-STORY — 소개·철학 좌우 분할

| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR002-STORY` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 14번 행(`COMP-SCR002-STORY`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **소개·철학 좌우 분할**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-002 | `/about` | `src/components/scr002/StorySection.tsx` |
...
신규 생성: `src/components/scr002/StorySection.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 확정 소개문·시작한 이유·철학·편집 원칙 4문단이 줄임 없이 표시

## Visual AC

- 좌 목차/우 문단 분할
- Mobile 세로 스택(D-001 §9)
