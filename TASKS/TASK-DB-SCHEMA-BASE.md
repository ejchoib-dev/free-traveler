...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 46번 행(`DB-SCHEMA-BASE`, Category=DB)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **DB 스키마(6개 테이블)**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
REQ-FUNC-029, REQ-FUNC-031, REQ-FUNC-034, REQ-FUNC-039, REQ-FUNC-040, REQ-FUNC-077

## Screen / Route / Page Entry

| Screen | Route | Page Entry |
|---|---|---|
| 해당없음 | N/A | `supabase/migrations/0001_schema.sql` |
...
신규 생성: `supabase/migrations/0001_schema.sql`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- `USER_PROFILE`, `MATE_POST`, `MATE_APPLICATION`, `USER_BLOCK`, `REPORT`, `OUTBOUND_URL_SETTING` **정확히 6개 테이블**만 생성(그 외 테이블 생성 금지)
...
- TC-FUNC-029: `REQ-FUNC-029` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
