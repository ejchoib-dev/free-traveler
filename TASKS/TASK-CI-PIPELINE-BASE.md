# CI-PIPELINE-BASE — CI 파이프라인(타입체크/린트/유닛/E2E)
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 57번 행(`CI-PIPELINE-BASE`, Category=CI)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **CI 파이프라인(타입체크/린트/유닛/E2E)**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| 해당없음 | N/A | `.github/workflows/ci.yml`(또는 동등 CI 설정) |
...
- `tsc --noEmit`, `next lint`, 위 Unit/RLS/E2E Task가 main 병합 전 통과
...
- 무인 자동 Merge를 수행하지 않는다(사람이 검토·승인
- SRS REQ-NF-031)

## Test Cases

- TC-NF-031: `REQ-NF-031` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- 추가 검증: 해당 없음(이 Task 자체가 게이트)에서 이 Task의 결과를 다시 확인

## Verify

해당 없음(이 Task 자체가 게이트)
