...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 56번 행(`E2E-MATE-AUTH`, Category=E2E_TEST)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **인증·동행 작성·참가·승인 Smoke**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-003/004/005 | `/travel-tools`, `/mates`, `/account` | `tests/e2e/mate-auth.spec.ts` |

## Design Ref

- `design-reference/UI_CONTRACT.md` — SCR-003/004/005 절(영역 순서·주요 Component·상태·이동·금지 기능)
- `design-reference/D-001/DESIGN.md` §19 — SCR-003/004/005 Section 순서와 최소 콘텐츠 수

## Depends On

PAGE-SCR003, PAGE-SCR004, PAGE-SCR005, DB-SEED-BASE
...
- 비회원 차단→로그인/성인확인→동행글 작성→참가 요청→작성자 승인까지 흐름을 Chromium으로 검증
