# SHARED-ERROR-PAGES — 404/500 복구 화면

| 항목 | 값 |
|---|---|
| Task ID | `SHARED-ERROR-PAGES` |
| Category | SHARED |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 42번 행(`SHARED-ERROR-PAGES`, Category=SHARED)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **404/500 복구 화면**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| 기술 Route | `*` | `src/app/not-found.tsx` / `src/app/error.tsx` |
...
신규 생성: `src/app/not-found.tsx`, `src/app/error.tsx`
...
- TC-FUNC-078: `REQ-FUNC-078` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
