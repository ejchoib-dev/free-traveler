...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 12번 행(`COMP-SCR002-HERO`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **Profile Hero**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-002 | `/about` | `src/components/scr002/ProfileHero.tsx` |
...
신규 생성: `src/components/scr002/ProfileHero.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 대표명·`50+ Trips`·`30+ Countries`·소개문 첫 문단
- 문의·SNS 링크(빈 링크 미렌더
- 허용 프로토콜만)

## Visual AC

- Hero 최대 520px(D-001 §17)

## Security/Privacy AC

- 허용 프로토콜(https/mailto)만 렌더
