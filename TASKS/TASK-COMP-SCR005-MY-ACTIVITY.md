...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 35번 행(`COMP-SCR005-MY-ACTIVITY`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **내 활동(내 글/참가요청/차단/즐겨찾기)**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-005 | `/account` | `src/components/scr005/MyActivityTab.tsx` |
...
신규 생성: `src/components/scr005/MyActivityTab.tsx`
...
- 내 글 목록(상태 배지
- 클릭 시 SCR-004 상세 이동)+"새 동행글 작성" CTA
- 참가 요청 목록(PENDING/ACCEPTED/REJECTED)
- 차단 목록+해제
- 즐겨찾기 목록(중복 생성 없음)
- 조회 중 목록 스켈레톤 표시(Loading, `design-reference/UI_CONTRACT.md` SCR-005 상태 행)

## Visual AC

- 목록형, 0건은 완성형 Empty State
...
- TC-FUNC-068: `REQ-FUNC-068` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
