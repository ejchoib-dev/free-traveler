...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 36번 행(`COMP-SCR005-ADMIN`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **관리자(신고 상태·외부 URL 설정)**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-005 | `/account` | `src/components/scr005/AdminTab.tsx` |
...
신규 생성: `src/components/scr005/AdminTab.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 신고 목록(OPEN/REVIEWING/RESOLVED/DISMISSED 필터)
- 항공·호텔 외부 URL 현재값+HTTPS 허용목록 검증 수정 폼(HTTP/javascript/data URL 저장 차단)
- URL 저장 실패(검증 실패·서버 오류) 시 값을 유지한 채 오류 메시지 표시(Error, `design-reference/UI_CONTRACT.md` SCR-005 상태 행)

## Visual AC

- 단순 목록/폼만(복잡한 분석/KPI 대시보드 금지
- D-001 §21)

## Security/Privacy AC

- Admin 역할만 접근(RLS+서버 역할 검증)
...
- 복잡한 분석/KPI 대시보드를 만들지 않는다 — 신고 상태 변경과 외부 URL 설정 두 가지 단순 목록/폼만.
