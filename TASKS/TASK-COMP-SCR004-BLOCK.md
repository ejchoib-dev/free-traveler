# COMP-SCR004-BLOCK — 차단

| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR004-BLOCK` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 30번 행(`COMP-SCR004-BLOCK`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **차단**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-004 | `/mates` | `src/components/scr004/BlockAction.tsx` |
...
신규 생성: `src/components/scr004/BlockAction.tsx`
...
- 차단·해제 액션
- 차단 후 상호 글·프로필·요청 비노출
- 비로그인 사용자가 차단을 시도하면 액션을 막고 로그인·성인인증 안내(→SCR-005)로 전환(Unauthorized, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
- 처리 실패(네트워크 오류·서버 오류) 시 오류 메시지+재시도 버튼 표시(Error, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
