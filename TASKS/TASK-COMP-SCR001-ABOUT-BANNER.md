...
| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR001-ABOUT-BANNER` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 10번 행(`COMP-SCR001-ABOUT-BANNER`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **대표 소개 CTA Banner**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-001 | `/` | `src/components/scr001/AboutBanner.tsx` |
...
신규 생성: `src/components/scr001/AboutBanner.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- `free_traveler`, `50+ Trips`, `30+ Countries`가 대표 페이지와 정확히 일치, "free_traveler 소개 보기" CTA → `/about`

## Visual AC

- CTA Banner 패턴(D-001 §5)
