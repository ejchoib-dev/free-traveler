...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 52번 행(`UNIT-MATE-STATE`, Category=UNIT_TEST)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **동행 상태 전이 단위 테스트**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| 해당없음 | N/A | `tests/unit/mate-state.spec.ts` |
...
- MATE_POST(OPEN/CLOSED/HIDDEN/DELETED)
- MATE_APPLICATION(PENDING/ACCEPTED/REJECTED/WITHDRAWN) 상태 전이가 `02_SRS_BASELINE.md` §6.5 상태 다이어그램과 일치
