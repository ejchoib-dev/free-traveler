# COMP-SCR004-REPORT — 신고

| 항목 | 값 |
|---|---|
| Task ID | `COMP-SCR004-REPORT` |
| Category | COMPONENT |
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 29번 행(`COMP-SCR004-REPORT`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **신고**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-004 | `/mates` | `src/components/scr004/ReportDialog.tsx` |
...
DB-ACCESS, SHARED-AUTH-SETUP

## Expected Files

신규 생성: `src/components/scr004/ReportDialog.tsx`
...
- 사유 코드+설명 제출
- 신고 ID·접수 시각 3초 이내 표시
- 비로그인 사용자가 신고를 시도하면 제출을 막고 로그인·성인인증 안내(→SCR-005)로 전환(Unauthorized, `design-reference/UI_CONTRACT.md` SCR-004 상태 행)
