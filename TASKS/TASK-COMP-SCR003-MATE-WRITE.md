# COMP-SCR003-MATE-WRITE — 동행 작성 / 로그인·성인인증 안내
...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 23번 행(`COMP-SCR003-MATE-WRITE`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **동행 작성 / 로그인·성인인증 안내**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-003 | `/travel-tools` | `src/components/scr003/MateWriteTab.tsx` |
...
COMP-SCR003-INTRO-TABS, DB-ACCESS, SHARED-AUTH-SETUP

## Expected Files

신규 생성: `src/components/scr003/MateWriteTab.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 미인증: 안내 카드+로그인/가입 CTA(→`/account`). 인증됨: 제목/국가/지역/기간/모집인원/스타일/설명/안전수칙 동의 폼
- 연락처 패턴 탐지 시 제출 차단(수정 안내 포함)

## Visual AC

- 미인증 상태는 완성형 Empty State(D-001 §14)

## Security/Privacy AC

- 인증·성인확인은 서버(Server Action)에서 재검증(REQ-FUNC-027,028)
...
- TC-FUNC-031: `REQ-FUNC-031` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-032: `REQ-FUNC-032` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-080: `REQ-FUNC-080` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- 추가 검증: UNIT-CONTACT-DETECTION, E2E-MATE-AUTH에서 이 Task의 결과를 다시 확인
...
- [ ] 'Verify'에 지정된 UNIT-CONTACT-DETECTION, E2E-MATE-AUTH가 통과한다
