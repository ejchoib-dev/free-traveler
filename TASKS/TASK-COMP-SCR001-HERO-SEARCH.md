...
이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 2번 행(`COMP-SCR001-HERO-SEARCH`, Category=COMPONENT)을 실제 개발 가능한 단위로 구체화한 것이다. 제목: **검색 Hero**. 근거 문서: `docs/06_SRS_UIUX_REVISED.md`(Requirement 원문), `design-reference/UI_CONTRACT.md`/`design-reference/D-001/DESIGN.md`(디자인 계약).
...
| Screen | Route | Page Entry |
|---|---|---|
| SCR-001 | `/` | `src/components/scr001/HeroSearch.tsx` |
...
신규 생성: `src/components/scr001/HeroSearch.tsx`

이 Task는 위에 적힌 파일 **이외의 어떤 파일도 수정하지 않는다**(§Forbidden 참조). 파일 생성 직전에 실제 트리를 다시 확인해 "신규 생성"/"수정"을 재판단한다.

## Functional AC

- 키워드 검색(한글 부분일치) + 국가·도시·계절·테마·기간 필터가 AND 조건으로 동작

## Visual AC

- Hero 높이 Desktop 최대 520px
- 다음 Section 상단이 스크롤 없이 보임(D-001 §17)
...
- TC-FUNC-002: `REQ-FUNC-002` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- TC-FUNC-003: `REQ-FUNC-003` 수용 기준을 검증(`docs/06_SRS_UIUX_REVISED.md` 해당 행 Acceptance Criteria 참조)
- 추가 검증: UNIT-TRAVEL-DATES(필터 날짜 검증 부분)에서 이 Task의 결과를 다시 확인

## Verify

UNIT-TRAVEL-DATES(필터 날짜 검증 부분)
